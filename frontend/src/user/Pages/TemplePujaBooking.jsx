...
const TemplePujaBooking = () => {
  ...
  const getDharmicTotal = () => {
    let sum = 0;
    if (donations.vastra) sum += 251;
    if (donations.annadan) sum += 501;
    if (donations.deepdan) sum += 101;
    if (donations.brahmin) sum += 1100;
            ...
            <div className="bg-white rounded-[2rem] shadow-sm overflow-hidden border border-orange-200">
              <div className="relative h-64 md:h-80 bg-gray-200">
                ...
                <div className="absolute bottom-8 left-8 text-white">
                  <h1 className="text-3xl md:text-5xl font-bold mb-2 tracking-tight">Mahashivratri Grand Puja</h1>
                  <div className="flex flex-wrap items-center gap-4 text-orange-200 text-sm font-bold">
                     <span className="flex items-center gap-1.5"><MapPin size={16} /> Varanasi</span>
                     <span className="flex items-center gap-1.5"><Calendar size={16} /> Wed, Jan 28</span>
                  </div>
                </div>
              </div>
              <div className="p-8">
                <p className="text-gray-600 font-medium leading-relaxed max-w-2xl">
                  Experience the divine energy of Lord Shiva in the heart of Kashi. Join thousands of devotees in this sacred night-long celebration.
                </p>
              </div>
            </div>

            {/* 2. BENEFITS & INCLUSIONS */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {[
                      { title: "Spiritual Upliftment", desc: "Divine energy from temples", icon: <Heart size={16}/> },
                      { title: "Divine Protection", desc: "Blessings for family", icon: <ShieldCheck size={16}/> }
                    ].map((benefit, i) => (
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {["Live HD streaming", "Real-time darshan", "7-day recording", "Personal Sankalp"].map((item, i) => (
                ...
                {dharmicItems.map((item) => (
                  <div
                    ...
                    className={`p-5 flex justify-between items-center transition-all cursor-pointer rounded-xl border-2 ${
                      donations[item.id] ? 'border-orange-500 bg-orange-50/30' : 'border-orange-200 bg-white hover:border-orange-300'
                    }`}
                  >
          ...
          <aside className="lg:col-span-4">
            <div className={`sticky top-8 p-8 ${mainCardStyle} !rounded-[1.5rem] shadow-xl`}>
              ...
              <div className="grid grid-cols-3 gap-2 mb-8">
                {tickets.map((t) => (
                  <button
                    ...
                    className={`flex flex-col items-center py-4 rounded-xl transition-all border-2 ${
                      selectedTicket === t.label 
                      ? 'border-orange-500 bg-orange-50/50 text-orange-600 shadow-sm' 
                      : 'border-orange-100 bg-gray-50/50 text-gray-400 hover:border-orange-200'
                    }`}
                  >
                    <Users size={16} />
              ...
              <div className="space-y-4 pt-6 border-t border-orange-200">
                <div 
                  onClick={() => dharmicRef.current?.scrollIntoView({ behavior: 'smooth' })}
                  className="cursor-pointer group flex items-center justify-between p-3 rounded-xl border border-orange-200 bg-orange-50/30 hover:bg-orange-50 transition-all"
                >
                  <div className="flex items-center gap-2 text-orange-600 font-black uppercase text-[10px] tracking-widest">
                    <Heart size={14} fill="currentColor" /> Dharmic Total
                  </div>
                  <span className="font-black text-gray-900 text-sm">₹{getDharmicTotal()}</span>
                </div>
                ...
                <button className="w-full bg-gradient-to-r from-orange-400 to-orange-500 text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all hover:shadow-orange-200">
                  <span>Pay Now</span>
                  <ArrowRight size={18} />
                </button>
                
                <p className="text-[9px] text-center font-bold text-gray-400 uppercase tracking-widest mt-4">
