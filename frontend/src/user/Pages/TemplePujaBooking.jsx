...
{/* SIDEBAR SUMMARY (EXACT IMAGE STYLE WITH DONATION) */}
<aside className="lg:col-span-4 lg:sticky lg:top-[100px] self-start z-30">
  <div className="bg-white rounded-3xl border border-orange-100 p-6 shadow-xl shadow-orange-50/50 space-y-6">
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {tickets.map((t) => (
          ...
          >
            {/* {selectedTicket === t.label && (
              <div className="absolute top-2 right-2 w-3 h-3 bg-orange-500 rounded-full border-2 border-white flex items-center justify-center">
                <CheckCircle size={8} className="text-white" />
              </div>
            )} */}
            <div className={`mb-2 p-2.5 rounded-xl ${
              selectedTicket === t.label ? "bg-orange-500 text-white shadow-md" : "bg-gray-50 text-gray-400"
            }`}>
              {t.icon}
            </div>
    ...
    <div className="pt-2 space-y-4">
      ...
      {/* Dharmic Contribution Sum */}
      <div className="flex items-center justify-between group cursor-pointer" onClick={() => scrollToSection("contributions")}>
        ...
        {/* Yahan Contribution ka Total Sum dikhega */}
        {Object.values(donations).some(val => val) && (
      ...
      {/* Temple Donation Checkbox (Image Style) */}
      <div className="flex items-center justify-between py-3 border-t border-gray-50">
        <label className="flex items-center gap-3 cursor-pointer">
          <input 
            type="checkbox" 
            checked={donations['temple_extra']} // Maan lete hain ye id hai
            onChange={(e) => setDonations(prev => ({...prev, 'temple_extra': e.target.checked}))}
            className="w-4 h-4 accent-orange-500 rounded"
          />
          <span className="text-[14px] text-gray-600 font-medium">Temple Donation</span>
        </label>
        <span className="text-[14px] font-bold text-orange-500">+₹1</span>
      </div>

      {/* 3. Final Total Display */}
      <div className="pt-4 border-t-2 border-dashed border-gray-100">
        <div className="flex justify-between items-center">
          <span className="text-[20px] font-bold text-gray-900">Total</span>
          <span className="text-[26px] font-black text-orange-600">
            ₹{calculateTotal().toLocaleString()}
          </span>
        </div>
        <p className="text-[12px] text-orange-500 font-bold text-center mt-3">
          58 spots remaining
        </p>
      </div>
    </div>
